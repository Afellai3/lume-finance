...
function MovimentoForm({ movimento, onClose, onSuccess }: MovimentoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show scomposizione modal if present
      if (result.scomposizione && result.scomposizione.componenti && result.scomposizione.componenti.length > 0) {
        setScomposizione(result.scomposizione);
        // Don't call onSuccess yet - wait for modal close
      } else {
        // No scomposizione, close immediately
        onSuccess();
      }
    } catch (error) {
  ...
  const handleScomposizioneClose = () => {
    ...
    <>
      ...
      {scomposizione && (
        <ScomposizioneModal
          ...
          onClose={handleScomposizioneClose}
        />
      )}
    </>
  );
