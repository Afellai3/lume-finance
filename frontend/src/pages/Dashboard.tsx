...
function Dashboard() {
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Errore di connessione al backend. Verifica che sia avviato.';
      setError(errorMsg);
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const [trend, comparison, warnings, topSpeseData] = await Promise.all([
        api.get(`/api/analytics/trend?period=${apiPeriod}`).catch(() => []),
        api.get('/api/analytics/comparison?period=month').catch(() => null),
        api.get('/api/analytics/budget-warnings').catch(() => []),
        api.get(`/api/analytics/top-spese?limit=5&period=${apiPeriod === '1m' ? 'month' : apiPeriod}`).catch(() => [])
      ]);
      ...
      console.log('✅ Analytics loaded:', { trend, comparison, warnings, topSpeseData });
    } catch (err) {
